...
export class CampaignService {
    ...
    create(data): Observable<CampaignModel> {
        ...
        .pipe(
            tap(_campaign => {
                this.messageService.info(`Campaign "${_campaign.name}" is create succesfully.`);
            }),
            catchError(this.errorsService.handleError<CampaignModel>(`Error`))
        )
    }
    ...
    stop(id: string): Observable<CampaignModel> {
        ...
        return this.http.post<CampaignModel>(url, {}).pipe(
            tap(_campaign => {
                this.messageService.info(`Campaign "${_campaign.name}" is stopped.`);
            }),
            catchError(this.errorsService.handleError<CampaignModel>(`Error`))
        )
    }
    ...
    active(id: string):  Observable<CampaignModel> {
        ...
        return this.http.post<CampaignModel>(url, {}).pipe(
            tap(_campaign => {
                ...
                this.messageService.info(`Campaign "${_campaign.name}" is activated.`);
            }),
            catchError(this.errorsService.handleError<CampaignModel>(`Error`))
        )
    }
    ...
    update(id: string, data: any) {
        ...
        return this.http.patch<CampaignModel>(url, data).pipe(
            tap(_campaign => {
                this.messageService.info(`Campaign "${_campaign.name}" is updated.`)
            }),
            catchError(this.errorsService.handleError<CampaignModel>(`Error`))
        )
    }
